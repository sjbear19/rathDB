...
std::string CoinRecord::serialize(std::shared_ptr<CoinRecord> coin_record) {
    auto* pcoin_record = new PCoinRecord();
    pcoin_record->set_version(coin_record->version);
    pcoin_record->set_is_coin_base(coin_record->is_coin_base);
    pcoin_record->set_height(coin_record->height);
...
std::shared_ptr<CoinRecord> CoinRecord::deserialize(const std::string& serialized_coin_record) {
    auto* pcoin_record = new PCoinRecord();
    ...
    std::shared_ptr<CoinRecord> coin_record = std::make_unique<CoinRecord>(
            pcoin_record->version(),
            pcoin_record->is_coin_base(),
            pcoin_record->height(),
            utxo,
            amounts,
            public_keys);
